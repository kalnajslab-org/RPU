 ...
 * RPURecordBuffer.h
 *
 * Fixed-capacity FIFO of encoded RPURecord records, held in RAM until they
...
#define RPU_MAX_RECORDS 5000

class RPURecordBuffer {
public:
    static constexpr size_t RECORD_SIZE = RPU_RECORD_BYTES;
    using EncodedRecord = etl::array<uint8_t, RECORD_SIZE>;

    // Encodes record and pushes it onto the FIFO. Returns false if full.
    bool push(const RPURecord& record);
...
private:
    etl::queue<EncodedRecord, RPU_MAX_RECORDS> records_;
};
